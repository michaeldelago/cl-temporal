...
grpc_call *lisp_grpc_channel_create_call(grpc_channel *channel,
  ...
  if (deadline_seconds != nullptr) {
    deadline = grpc_timeout_seconds_to_deadline(*deadline_seconds);
  } else {
...
void *new_tag(int num) { return new int(num); }
...
grpc_metadata_array *lisp_grpc_op_get_initial_metadata(grpc_op *ops,
                                                       int index) {
...
grpc_metadata_array *lisp_grpc_op_get_trailing_metadata(grpc_op *ops,
                                                        int index) {
...
void lisp_grpc_server_make_send_status_op(grpc_op *op, int index,
...
void lisp_grpc_make_send_status_from_server_op(grpc_op *op, int index,
                                               grpc_metadata *trailing_metadata,
                                               uint32_t metadata_count,
                                               grpc_status_code status,
                                               uint32_t flags) {
...
grpc_slice *create_empty_grpc_slice() { return new grpc_slice(); }
...
void free_grpc_slice(grpc_slice *slice) { free(slice); }
